...
- `dashboard.html` — a bold **urgent alerts banner** at the very top for time-critical items (e.g. "pay this before it's abandoned"), separate from everything else and dismissible; Bit/Cash/**Card** balances, **estimated inventory value** (not cost — see below), in-transit value, this month's spend/revenue/profit, plus insight cards: purchases stuck in Ordered/Shipped 14+ days, inventory sitting unsold 21+ days, forwarder shipments unpaid or running late, and **Deals & drops** (coupons/new releases found by the daily market scan, dismissible)
- `purchases.html` — log a purchase (photo/video/text → AI draft → review → confirm), list, status changes, **edit in place** (reverses and reapplies its stock/cash effect, including adding/removing/re-splitting line items — this is how a multi-item order gets corrected into separate inventory items too), **delete** (also reverses stock/cash — open any purchase → "Delete purchase"), an optional **order link** (e.g. the Vinted listing) shown as a "↗" in the list and a full link on the detail page, and **multi-currency support** — pick a currency, enter that purchase's exchange rate by hand, and everything downstream (cost basis, cash ledger, dashboard sums) is stored in ILS while the original amount stays visible for reference
- `inventory.html` — search/**filter by type**/**sort** (name, value, cost, quantity, or oldest-in-stock) stock, edit an item, upload/replace a photo per item (also auto-set the first time a new item is created from a purchase photo), manual quantity adjustments (with a required reason, logged to `stock_adjustments`), **estimated resale value per item** — separate from cost basis (which stays accurate to what you paid, for profit math), defaults to 1.9× the item-only cost, tell it the real number whenever you know one — **"+ Add item"** to enter stock directly (name, quantity, cost, optional value/photo) without it being a purchase or sale (no cash effect) — and a **"Sell"** link straight to a pre-filled sale for that item
...
- `orders.html` — every open order in one place (awaiting seller → at forwarder → forwarder unpaid → in transit to you → received), **showing each package's contents** (the items inside, not just source/total), plus a "Forwarder shipments" section to bundle purchases sitting at Redbox/MyUS into one outbound shipment, mark it paid, add tracking/expected-arrival, and mark it arrived (flips every bundled purchase to received in one go)
- `vinted-activity.html` — a read-only feed of Vinted seller offers (received/accepted/declined) and message notifications, synced daily from Vinted's own emails; filterable by kind. Not interactive (no Vinted API) — it's there so nothing gets missed, not to reply from
- `supabase/migrations/0001_init.sql` … `0009_purchase_source_url.sql` — full schema, RLS, seed data, two fixes Supabase's own advisor flagged after the first migration, the `currency`/`fx_rate` columns on `purchases`, the `forwarder_shipments` table, a third `card` payment account, `app_settings` (holds the Vinted-sync EUR/ILS rate and the default value multiplier), `purchases.source_ref` (dedup key for the email sync), `market_alerts`, `inventory_items.avg_item_cost`/`estimated_value` (estimated resale value, separate from cost basis), and `purchases.source_url`
- A **daily scheduled task** (`pokii-vinted-and-deals-sync`, runs 08:09 local) that reads Vinted's order-confirmation emails from your connected Gmail and imports new ones as purchases (skipping anything already imported or later cancelled/refunded); separately reads Redbox's transactional emails and keeps `forwarder_shipments` in sync (creation, shipped, delivered, and a loud flag if a package is at risk of being declared abandoned — payment amount/status stays something you set by hand, since Redbox's emails don't state it); and separately scans the web for current Pokémon card / Funko coupons, deals, and drops, posting genuine finds to the dashboard's Deals & drops card. It only fires while the Claude app is open — closed at the scheduled time means it runs on next launch instead. No equivalent MyUS emails were found in the connected inbox, so MyUS shipments still need to be tracked by hand via `orders.html`.
...
- The platform only supports one Gmail connector at a time, so the second inbox (your business partner's) forwards its Vinted/Redbox mail into the primary one via a Gmail filter rather than being connected directly — this means both accounts' orders/tracking/offers show up correctly, but the connector itself always needs to stay pointed at `amitcohen0411@gmail.com` for the daily sync to work for both of you.
