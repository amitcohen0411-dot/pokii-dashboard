...
import { setInventoryImage } from "./inventoryMatch.js";
    ...
    .map((item, i) => {
      ...
      <div class="list-row" style="cursor:pointer" data-id="${item.id}">
        ...
        <div style="text-align:right">
          ...
          <div class="meta">avg ${money(item.avg_unit_cost)}</div>
        </div>
      </div>`;
    })
    .join("");
...
async function openDetail(id) {
  ...
  body.innerHTML = `
    ...
    <h3 style="margin-top:24px">Stock</h3>
    <p class="meta">Currently <strong>${item.quantity}</strong> in stock at avg cost ${money(item.avg_unit_cost)} (total ${money(item.quantity * item.avg_unit_cost)})</p>
    ${item.quantity > 0 ? `<p class="meta">Sitting since last restock: ${daysAgo(item.updated_at)} day(s)</p>` : ""}
    <label for="i-adjust">Adjust quantity by (use a negative number to reduce)</label>
